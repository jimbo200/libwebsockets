...
lws_jwe_encrypt_rsa_aes_gcm(struct lws_jwe *jwe, char *temp, int *temp_len)
{
	...
	int n, ret = -1, used = 0, ot = *temp_len;
	...
	if (lws_jws_randomize_element(jwe->jws.context, &jwe->jws.map, LJWE_EKEY,
				      temp + (ot - *temp_len), temp_len,
				      ekbytes, 0))
		return -1;

	/* we must cleanse enc_cek before exiting, note the length */
	used = jwe->jws.map.len[LJWE_EKEY];
	memcpy(enc_cek, jwe->jws.map.buf[LJWE_EKEY], jwe->jws.map.len[LJWE_EKEY]);
	...
	memcpy((void *)jwe->jws.map.buf[LJWE_EKEY], enc_cek,
